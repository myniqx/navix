...
export function MediaCard({ fKey, item, variant }: MediaCardProps) {
  ...
  const setCache = (s: CacheState) => { cacheStateRef.current = s; setCacheState(s); };
  const setPlay = (s: PlayState) => { playStateRef.current = s; setPlayState(s); };
  ...
  const reset = () => {
    if (cacheTimerRef.current) { clearInterval(cacheTimerRef.current); cacheTimerRef.current = null; }
    if (playTimerRef.current) { clearInterval(playTimerRef.current); playTimerRef.current = null; }
    setCache('idle');
    setPlay('idle');
    setCacheProgress(0);
  ...
  const topLeftBadge = variant === 'movie'
    ? `★ ${getRating(item)}`
    : variant === 'live'
      ...
      >
        ...
        {isPlaying && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <div style={{ fontSize: 18, color: accent, textShadow: `0 0 12px ${accent}99` }}>▶</div>
            <div style={{ fontSize: 8, color: accent, fontWeight: 700, letterSpacing: '0.1em' }}>{playLabel}</div>
          </div>
        )}

        {/* Pending indicator */}
        {isPending && (
          <div style={{ fontSize: 9, color: '#4fc3f7', fontWeight: 600 }}>⟳ Loading...</div>
        )}
