  ...
  const handlePlayerEvent = (e: { type: string; action: string }): boolean => {
    ...
    if (
      ...
      setPlayer((p) => {
        ...
        if (!nextPaused) {
          // Switching to play: show play icon for 2s
          ...
          // Switching to pause: hide play icon if visible
          if (playIconTimerRef.current !== null) {
            clearTimeout(playIconTimerRef.current);
      ...
      return false; // let MultiLayer also process (trapped anyway)
    }
    return false;
  };
...
function SidePanel({ fKey, onEvent, close }: MultiLayerPanelProps) {
...
function ControlsPanel({
  ...
  return (
    ...
    >
      <VerticalList fKey={fKey} onEvent={onEvent}>
