 ...
 * ContentCard
 *
 * A focusable media card built on <NavixExpandable>.
 *
 * COLLAPSED state (isExpanded = false):
 *   - Shows poster + "▶ Play" bar
 *   - directlyFocused = true when this card is the active leaf
 *   - Enter → NavixExpandable opens (isExpanded = true)
 *   - Left/right/up/down → bubble to parent NavixHorizontalList / NavixVerticalList
 *
 * EXPANDED state (isExpanded = true):
 *   - Shows poster + action buttons (Play, Info)
 *   - Inner NavixHorizontalList mounts, its children register into the focus tree
 *   - Left/right → navigate between action buttons (caught by inner NavixHorizontalList)
 *   - Back → NavixExpandable closes (core ExpandableBehavior handles this)
        ...
        return (
          ...
          >
            ...
            {!isExpanded ? (
              ...
              // Action overlay — NavixHorizontalList mounts two NavixButton nodes into the tree.
              // left/right navigates between them, back collapses via ExpandableBehavior.
              <NavixHorizontalList fKey={`${fKey}-actions`}>
